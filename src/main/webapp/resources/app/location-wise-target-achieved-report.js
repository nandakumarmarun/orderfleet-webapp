...
if (!this.LocationWiseTargetAchievedReport) {
	this.LocationWiseTargetAchievedReport = {};
}

(function() {
	'use strict';

	var locationWiseTargetAchievedReportContextPath = location.protocol + '//'
	...
	function loadLocationWiseTargetAchievedReport() {
		...
		$.ajax({
			url : locationWiseTargetAchievedReportContextPath + "/load-data",
			type : 'GET',
			data : {
	...
	function fillTargetData(targetData) {
		...
		var performanceRow = "";
		$.each(targetData.productGroupLocationTargets, function(locationName, targets) {
