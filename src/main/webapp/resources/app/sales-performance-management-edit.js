											...
											$('#tblVoucherDetails')
													.append(
															"<tr data-id='"
																	...
																	+ "</td><td>"
																	+ (voucherDetail.editOrder ? "<div class='input-group'><input type='number' onchange ='InventoryVoucher.saveUp(\""
																	        + pid
																        	+ "\",\""
																	        + voucherDetail.detailId
																	        + "\",\""
																	        + voucherDetail.qntyDiff
																	        + "\");' id='updatedQty-"
																			+ voucherDetail.detailId
...
//																	        + "\");'  value='Save'></div></div>"
						...
						$('#savebtn')
						.html(
								"<button id='myFormSubmitee' class='btn btn-success' onClick='InventoryVoucher.saveUpdatelol(\""
																	        + pid
																        	+ "\");'>Save And Confirm</button>"
																        	+"<button id='myFormCancle' type='button' class='btn btn-default' data-dismiss='modal'>Cancel</button>");
	...
	InventoryVoucher.saveUp = function(ivhPid, ivdId, pdiff) {
		var qntyorg = Number($('#updatedQty-' + ivdId).val());
		var qntdiff = Number(pdiff);
		if(qntyorg > qntdiff){
			alert("Please Enter Less Than Pending Quntity "+pdiff);
			return;
		}
		if(InventoryVoucherHeaderModel.pid == null){
			InventoryVoucherHeaderModel.pid = ivhPid;
		}
		if(InventoryVoucherHeaderModel.ivhd.length !== 0){
			InventoryVoucherHeaderModel.ivhd = InventoryVoucherHeaderModel.ivhd.filter(item => item.id !== ivdId)
		}
		InventoryVoucherHeaderModel.ivhd.push({
			"id" : ivdId,
			"qnty" : $('#updatedQty-' + ivdId).val(),
		})
		$(".error-msg").html("Saved..."+ $('#updatedQty-' + ivdId).val());
	}
	...
	$('#myFormSubmit').on(
			...
			function(newpid) {
				if(InventoryVoucherHeaderModel.pid == null){
					InventoryVoucherHeaderModel.pid == newpid;
				}
...
InventoryVoucher.saveUpdatelol = function (pid){
		if(InventoryVoucherHeaderModel.pid == null){
			
			InventoryVoucherHeaderModel.pid = pid;
		}
		if (confirm("Are you sure?") == true) {
