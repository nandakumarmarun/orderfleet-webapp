// Create a DocumentWiseYTDReport object only if one does not already exist. We create the
// methods in a closure to avoid creating global variables.

if (!this.DocumentWiseYTDReport) {
	this.DocumentWiseYTDReport = {};
}

(function() {
	'use strict';

	var documentWiseYTDReportContextPath = location.protocol + '//'
			+ location.host + location.pathname;

	$(document).ready(function() {

		loadAllDocumentsByDocumentType();
		...
		$("#btnApply").click(function() {
			...
			getReport(documentPid);
		});
	});

	function loadAllDocumentsByDocumentType() {
		var documentType = $('#dbDocumentType').val();
		if (documentType == "no") {
			$("#dbDocument").html("<option>Select Document</option>");
			alert("Please Select Document Type");
			return;
		}
		$("#dbDocument").html("<option>Documents loading...</option>")
		$.ajax({
			url : documentWiseYTDReportContextPath + "/load-document",
			type : 'GET',
			data : {
	...
	function getReport(documentPid) {
		$("#tblBody")
				.html(
		...
		$
				.ajax({
					url : documentWiseYTDReportContextPath + "/getReport",
					type : 'GET',
					data : {
