...
(function() {
	...
	function loadUserByCompanyId(companyid) {
		...
		$.ajax({
			url : companyUserDeviceContextPath + "/" + companyid,
			type : 'GET',
		...
		if (companyPid != "no") {
			...
			$.ajax({
				...
				success : function(companyUserDevices) {
					createTable(companyUserDevices);
				},
	...
	function createTable(companyUserDevices) {
		$('#tableCompanyUserDevice').html("");
							...
							$('#tableCompanyUserDevice')
									.append(
											"<tr><td>"
													...
													+ "</td><td>"
													+ companyUserDevice.buildVersion
													+ "</td></tr>");
						});
	}
