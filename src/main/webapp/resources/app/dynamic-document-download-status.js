...
(function() {
	...
	function submitUpdatedForms() {
		...
		$.ajax({
			method : 'POST',
			url : dynamicDocumentContextPath + "/filled-forms-download-status",
...
// }
						...
						$.each(dynamicDocuments,
										function(index, dynamicDocument) {
													...
													var content =	"<tr><td>"
																	...
																	+"</td><td>"
																	+spanStatus(dynamicDocument.pid,dynamicDocument.tallyDownloadStatus)
	...
	function spanStatus(documentVoucherPid, status) {

		var pending = "'" + 'PENDING' + "'";
		var processing = "'" + 'PROCESSING' + "'";
		var completed = "'" + 'COMPLETED' + "'";
		var spanStatus = "";
		var pid = "'" + documentVoucherPid + "'";
		switch (status) {
		case 'PENDING':
			spanStatus = '<div class="dropdown"><span class="label label-default dropdown-toggle" data-toggle="dropdown" style="cursor: pointer;">'
					...
					+ '<li onclick="DynamicDocument.setStatus('
					+ pid
					+ ','
					+ processing
	...
	DynamicDocument.setStatus = function(pid, tallyDownloadStatus) {

		if (confirm("Are you sure?")) {
			// update status;changeStatus

			$.ajax({
				url : dynamicDocumentContextPath + "/changeStatus",
				method : 'GET',
				data : {
					pid : pid,
