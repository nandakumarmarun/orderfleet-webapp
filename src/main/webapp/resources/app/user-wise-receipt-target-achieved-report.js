...
if (!this.UserWiseReceiptTargetAchievedReport) {
	this.UserWiseReceiptTargetAchievedReport = {};
}

(function() {
	'use strict';

	var userWiseReceiptTargetAchievedReportContextPath = location.protocol + '//'
	...
	$( document ).ready(function() {
		...
		$('#applyBtn').click(function(){
			loadUserWiseReceiptTargetAchievedReport();
		});
		
	});
	
	function loadUserWiseReceiptTargetAchievedReport() {
		$('#tblUserWiseReceiptTargetAchievedReport').css('display','none');
		...
		$.ajax({
			url : userWiseReceiptTargetAchievedReportContextPath + "/load-data",
			type : 'GET',
			data : {
			...
			success : function(response) {
				...
				if (response) {
					$("#tblUserWiseReceiptTargetAchievedReport").show();
					fillTargetData(response);
				} else {
					console.log("User Wise Receipt target or acheived not found for any employees....");
					var trRow = "<tr style='color:red; font-size: large;'><td><b>"
							+ "User Wise Receipt target or acheived not found for any employees"
							+ "</b></td></tr>";
					$("#tblBody").html(trRow);
					$("#tblUserWiseReceiptTargetAchievedReport").show();
				}
	...
	function fillTargetData(targetData) {
		$("#tblUserWiseReceiptTargetAchievedReport tbody").html("");
		...
		var performanceRow = "";
		$.each(targetData.userWiseReceiptTargets, function(userName, targets) {
