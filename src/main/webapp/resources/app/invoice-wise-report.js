...
(function() {
	...
	var invoiceWiseReportContextPath = location.protocol + '//'
					...
					$
						.each(
							...
							function(index, invoiceWiseReport) {
								...
								if(invoiceWiseReport.locationType=="GpsLocation" && invoiceWiseReport.latitude==0 || invoiceWiseReport.locationType=="TowerLocation" && invoiceWiseReport.latitude==0 || invoiceWiseReport.location=="Unable to find location"){
									locationName="<span class='btn btn-success'  id='"+invoiceWiseReport.pid+"' onClick='InvoiceWiseReport.getLocation(this)' >get location</span>";
								}
//								if(invoiceWiseReport.locationType=="GpsLocation" && invoiceWiseReport.latitude==0 ||  invoiceWiseReport.location=="Unable to find location"){
//									locationName="<span class='btn btn-success'  id='"+invoiceWiseReport.pid+"' onClick='InvoiceWiseReport.getLocation(this)' >get location</span>";
//								}
								...
								if (invoiceWiseReport.activityStatus == 'ACCEPTED') {
									...
									activityStatusTd = "<td id='rejectReason' title='" + invoiceWiseReport.rejectReasonRemark + "'><font color='red'>" + invoiceWiseReport.activityStatus + ""
										+ "</td>";
								}
								var changeAccount="";
								if(interimSave=="true"){
								...
								if(distanceTarvel=="true"){
									geoTag="&nbsp;&nbsp;<button type='button' title='geo tag to account' class='btn btn-orange entypo-map' aria-label='entypo-map' onclick='InvoiceWiseReport.setGeoTag(\""+ invoiceWiseReport.pid+"\",\""+invoiceWiseReport.accountProfileLatitude+"\",\""+invoiceWiseReport.accountProfileLongitude+"\",\""+invoiceWiseReport.accountProfileLocation+"\",\""+invoiceWiseReport.latitude+"\",\""+invoiceWiseReport.longitude+"\",\""+invoiceWiseReport.location+"\",this);'></button>";
								}
								...
								$('#tBodyInvoiceWiseReport')
									.append(
										...
										+ locationName
										+ locationIcon + "</i><br><label>Customer Location : </label><i id='lbl_"+ invoiceWiseReport.pid+ "'> "
										+ (invoiceWiseReport.accountProfileLocation == null ? "" : invoiceWiseReport.accountProfileLocation)
										+ "</i></td>"
										+ "<td>"
	...
	InvoiceWiseReport.setGeoTag= function(pid,accLat,accLon,accLoc,newLat,newLon,newLoc, obj){
	...
	InvoiceWiseReport.getActivities= function(activityType){
		...
		if(activityType=='all'){
		 ...
		 $.ajax({
			 url : invoiceWiseReportContextPath+"/getActivities/" + activityType,
			 method : 'GET',
	...
	InvoiceWiseReport.createAndChangeAccount=function(){
		...
		$.ajax({
			url : invoiceWiseReportContextPath + "/createAndChangeAccountProfile/"+savePidForAccountChanging+"/"+$("#field_territory").val(),
			type : "POST",
	...
	InvoiceWiseReport.getForms = function() {
		...
		$
				.ajax({
					url : invoiceWiseReportContextPath+ "/getForms/" + dynamicDocumentPid,
					method : 'GET',
					success : function(data) {
	...
	function showDynamicDocumentImages(pid) {
		$('#divInvoiceWiseReportImages').html('<img src="/resources/assets/images/content-ajax-loader.gif" style="display: block; margin: auto;">');
		$.ajax({
