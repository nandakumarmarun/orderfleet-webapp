...
if (!this.InvoiceWiseDuplicateReports) {
	this.InvoiceWiseDuplicateReports = {};
}

(function() {
	'use strict';

	var invoiceWiseDuplicateReportsContextPath = location.protocol + '//' + location.host
			+ location.pathname;
	...
	$(document).ready(function() {
		...
		if (filterBy) {
			...
			InvoiceWiseDuplicateReports.filter();
		}
...
// }
	...
	InvoiceWiseDuplicateReports.downloadXls = function() {
	...
	InvoiceWiseDuplicateReports.testdownloadXls = function() {
     ...
     var	inclSubordinate = $('#inclSubOrdinates').is(":checked");
      ...
      window.location.href =  invoiceWiseDuplicateReportsContextPath+"/downloadxls?&employeePid="+employeePid+'&documentPid='+documentPid+'&activityPid='+activityPid+
                                             '&filterBy='+filterBy+'&fromDate='+fromDate+'&toDate='+toDate+'&inclSubordinate='+inclSubordinate;

console.log("Success.....");
	}
	InvoiceWiseDuplicateReports.filter = function() {
		...
		$
				.ajax({
					url : invoiceWiseDuplicateReportsContextPath + "/filter",
					type : 'GET',
					data : {
										...
										function(index, invoiceWiseReport) {
											...
											if (invoiceWiseReport.location == "No Location"
												...
												locationName = "<span class='btn btn-success'  id='"
														...
														+ "' onClick='InvoiceWiseDuplicateReports.getLocation(this)' >get location</span>";
											}
											...
											if (invoiceWiseReport.towerLocation == "Not Found"
												...
												towerLocationName = "<span class='btn btn-success'  id='"
														+ invoiceWiseReport.pid
														+ "' onClick='InvoiceWiseDuplicateReports.getTowerLocation(this)' >get location</span>";
											}
											...
											var changeAccount = "";
											if (interimSave == "true") {
												changeAccount = "&nbsp;&nbsp;<button type='button' class='btn btn-blue' onclick='InvoiceWiseDuplicateReports.changeAccountProfile(\""
														+ invoiceWiseReport.pid
											...
											if (distanceTarvel == "true") {
												geoTag = "&nbsp;&nbsp;<button type='button' title='geo tag to account' class='btn btn-orange entypo-map' aria-label='entypo-map' onclick='InvoiceWiseDuplicateReports.setGeoTag(\""
														+ invoiceWiseReport.pid
														+ "\",\""
											...
											$
													.each(
															...
															function(index,
																...
																var editButton = "";
																if (interimSave == "true") {
																	editButton = "<button type='button' class='btn btn-white' onclick='InvoiceWiseDuplicateReports.editDetails(\""
																			+ invoiceWiseReport.pid
																			+ "\",\""
																...
																if (executionDetail.documentType != "DYNAMIC_DOCUMENT") {
																	$(
																			...
																			.append(
																					"<tr class='tableexport-ignore' style='background: rgba(225, 225, 225, 0.66);' data-id='"
																							...
																							+ "</td><td><button type='button' class='btn btn-white' onclick='InvoiceWiseDuplicateReports.viewDetails(\""
																							+ executionDetail.pid
																							+ "\",\""
																							+ executionDetail.documentType
																							+ "\");'>View Details</button>"
																	...
																	if (executionDetail.imageSaved) {
																		elmViewImage = "<td><button type='button' class='btn btn-blue' onclick='InvoiceWiseDuplicateReports.showModalPopup($(\"#imagesModal\"),\""
																				+ executionDetail.pid
	...
	InvoiceWiseDuplicateReports.setGeoTag = function(pid, accLat, accLon, accLoc, newLat,
			newLon, newLoc, obj) {
	...
	InvoiceWiseDuplicateReports.saveNewGeoLocation = function() {
		var pid = $("#hdn_lbl").val();
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/updateGeoTag/" + pid,
			method : 'GET',
	...
	InvoiceWiseDuplicateReports.getActivities = function(activityType) {
		...
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/getActivities/"
					+ activityType,
			method : 'GET',
	...
	InvoiceWiseDuplicateReports.editDetails = function(pid, documentType, documentTypePid) {
	...
	InvoiceWiseDuplicateReports.changeAccountProfile = function(pid) {
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/getDynamicDocument/" + pid,
			method : 'GET',
			async : false,
			success : function(data) {
				...
				InvoiceWiseDuplicateReports.getForms();
			},
			error : function(xhr, error) {
				onError(xhr, error);
			}
	...
	InvoiceWiseDuplicateReports.searchTable = function(inputVal, table) {
	...
	InvoiceWiseDuplicateReports.newAccount = function() {
		$("#newAccount").css("display", "block");
		$("#oldAccounts").css("display", "none");
	};

	InvoiceWiseDuplicateReports.oldAccount = function() {
		$("#oldAccounts").css("display", "block");
		$("#newAccount").css("display", "none");
	};

	InvoiceWiseDuplicateReports.changeAccount = function() {
		...
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/changeAccountProfile",
			type : "POST",
			data : {
	...
	InvoiceWiseDuplicateReports.createAndChangeAccount = function() {
		if ($("#field_name").val() == "") {
		...
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath
	...
	InvoiceWiseDuplicateReports.getForms = function() {
		...
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/getForms/"
					+ dynamicDocumentPid,
			method : 'GET',
			success : function(data) {
	...
	InvoiceWiseDuplicateReports.loadAccountFromForm = function() {
		$('#field_name').val("");
		$('#field_alias').val("");
		$('#field_city').val("City");
		$('#field_location').val("");
		...
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/getAccountFromForm",
			method : 'GET',
			data : {
				formPid : formPid,
	...
	InvoiceWiseDuplicateReports.getLocation = function(obj) {
		var pid = $(obj).attr("id");
		$(obj).html("loading...");
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/updateLocation/" + pid,
			method : 'GET',
			success : function(data) {
	...
	InvoiceWiseDuplicateReports.getTowerLocation = function(obj) {
		var pid = $(obj).attr("id");
		$(obj).html("loading...");
		$.ajax({
			url : invoiceWiseDuplicateReportsContextPath + "/updateTowerLocation/" + pid,
			method : 'GET',
			success : function(data) {
	...
	InvoiceWiseDuplicateReports.accept = function(pid, obj) {
		$.ajax({
	...
	InvoiceWiseDuplicateReports.reject = function() {
	...
	InvoiceWiseDuplicateReports.showModal = function(pid, obj) {
		object = obj;
	...
	InvoiceWiseDuplicateReports.viewDetails = function(pid, documentType) {
		if (documentType == "INVENTORY_VOUCHER") {
	...
	InvoiceWiseDuplicateReports.showModalPopup = function(el, pid, action) {
		if (pid) {
			switch (action) {
			case 0:
	...
	InvoiceWiseDuplicateReports.showDatePicker = function() {
		$("#txtFromDate").val("");
		$("#txtToDate").val("");
		if ($('#dbDateSearch').val() == "CUSTOM") {
	...
	InvoiceWiseDuplicateReports.closeModalPopup = function(el) {
		el.modal('hide');
	}
