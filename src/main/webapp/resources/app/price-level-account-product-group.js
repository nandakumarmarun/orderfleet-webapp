if (!this.PriceLevelAccountProductGroup) {
	this.PriceLevelAccountProductGroup = {};
}

(function() {
	'use strict';

	var priceLevelAccountProductGroupContextPath = location.protocol + '//'
			+ location.host + location.pathname;
	var createEditForm = $("#priceLevelAccountProductGroupForm");
	var deleteForm = $("#deleteForm");
	var priceLevelAccountProductGroupModel = {
		pid : null,
		accountPid : null,
	...
	PriceLevelAccountProductGroup.loadProductGroup = function() {
		...
		$
				.ajax({
					url : priceLevelAccountProductGroupContextPath
	...
	PriceLevelAccountProductGroup.saveAccountAndProductGroup = function() {
		var accountPids = [];
	...
	function createUpdatePriceLevelAccountProductGroup(el) {
		priceLevelAccountProductGroupModel.accountPid = $('#dbAccount').val();
		priceLevelAccountProductGroupModel.productGroupPid = $(
				'#dbProductGroup').val();
		priceLevelAccountProductGroupModel.priceLevelPid = $('#dbPriceLevel1')
				.val();
		...
		$.ajax({
			...
			data : JSON.stringify(priceLevelAccountProductGroupModel),
			success : function(data) {
				onSaveSuccess(data);
			},
			error : function(xhr, error) {
	...
	function showPriceLevelAccountProductGroup(pid) {
		$.ajax({
			url : priceLevelAccountProductGroupContextPath + "/" + pid,
			method : 'GET',
			success : function(data) {
	...
	function editPriceLevelAccountProductGroup(pid) {
		$.ajax({
			...
			success : function(data) {
				...
				// set pid
				priceLevelAccountProductGroupModel.pid = data.pid;
			},
			error : function(xhr, error) {
				onError(xhr, error);
			}
	...
	function onSaveSuccess(result) {
		...
		window.location = priceLevelAccountProductGroupContextPath;
	}
	...
	PriceLevelAccountProductGroup.showModalPopup = function(el, pid, action) {
		resetForm();
		if (pid) {
			switch (action) {
			case 0:
			...
			case 2:
				deleteForm.attr('action',
						priceLevelAccountProductGroupContextPath + "/" + pid);
				break;
			}
		}
		el.modal('show');
	...
	PriceLevelAccountProductGroup.closeModalPopup = function(el) {
		el.modal('hide');
	}
	...
	function resetForm() {
		...
		priceLevelAccountProductGroupModel.pid = null; // reset
