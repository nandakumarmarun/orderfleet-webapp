...
(function() {
	...
	function loadAccountsByLocation(locPid){
		$
		.ajax({
			url : contextPath + "/web/executive-wise-leads/accountProfile",
			type : 'GET',
			data : {
				locationPid : locPid
			},
	...
	function loadStagesByStageGroup(sgPid){
		$
		.ajax({
			url : contextPath + "/web/executive-wise-leads/stages",
			type : 'GET',
			data : {
				stageGroupPid : sgPid
			},
	...
	StageReport.filter = function() {
		let searchDate = getParameterByName('dbDateSearch');
		if(searchDate != null) {
			$('#dbDateSearch').val(searchDate);
		}
		...
		let  empPid = getParameterByName('user-key-pid');
		if(empPid == null || empPid == "") {
			empPid = $('#dbEmployee').val();
		}
		if(empPid=="no" || empPid == null) {
			$('#tBodyStageReport')
			.html(
	...
	function loadReports() {
		...
		$
				.ajax({
					url : contextPath + "/web/executive-wise-leads/filter",
					type : 'GET',
					data : {
						employeePid : empPid,
						stagePid : stagePids,
					...
					success : function(stages) {
						...
						var employeeName = $('#dbEmployee'). children("option:selected"). html();
						$.each(stages, function(indx, stage){
							var rowHtml = "<tr>";
							
							rowHtml += "<td>"+ employeeName +"</td>"
