...
function connect() {
	...
	stompClient.connect({}, function(frame) {
		stompClient.subscribe('/live-tracking-attendance/map-view/' + companyId, function(
				response) {
...
function loadUsersDetails(dashboardUserPid) {
	...
	$
			.ajax({
				url : contextPath + "/web/getUsersAttendanceDetails",
				type : 'GET',
				data : {
					date : $("#txtDate").val()
				},
...
function convertTime(createdDate) {
	if (createdDate) {
		return moment(createdDate).format('h:mm:ss a');
	} else {
		return "";
	}
}
