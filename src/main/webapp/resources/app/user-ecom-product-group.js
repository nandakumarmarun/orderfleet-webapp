// Create a UserEcomProductGroup object only if one does not already exist. We create the
// methods in a closure to avoid creating global variables.

if (!this.UserEcomProductGroup) {
	this.UserEcomProductGroup = {};
}

(function() {
	'use strict';

	var userEcomProductGroupContextPath = location.protocol + '//' + location.host
			+ location.pathname;
	var userEcomProductGroupModel = {
		userPid : null
	};
	...
	function loadUserProductGroup(userPid) {
		...
		userEcomProductGroupModel.userPid = userPid;
		// clear all check box
		$.ajax({
			url : userEcomProductGroupContextPath + "/" + userPid,
			type : "GET",
	...
	function saveAssignedProductGroups() {
		...
		$.ajax({
			url : userEcomProductGroupContextPath + "/save",
			type : "POST",
			data : {
				userPid : userEcomProductGroupModel.userPid,
	...
	UserEcomProductGroup.closeModalPopup = function(el) {
		el.modal('hide');
	}
