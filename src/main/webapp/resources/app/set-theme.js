...
(function() {
	...
	function saveSetTheme() {
		$.ajax({
			...
			method : 'POST',
			data : {
				themeColor : $('input[name=theme]:checked').val()
			},
	...
	function getRunningTheme() {

		$.ajax({
			url : contextPath + "/getCurrentTheme",
			method : 'GET',
			success : function(data) {
				if (data.value != null) {
					var value = data.value.split(".");
					$('#' + value[0]).addClass('checked');
				}
				else{
					$('#white').addClass('checked');
				}
			},
			error : function(xhr, error) {
				onError(xhr, error);
	...
	SetTheme.closeModalPopup = function(el) {
		el.modal('hide');
	}
