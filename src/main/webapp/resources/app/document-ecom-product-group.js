// Create a DocumentEcomProductGroup object only if one does not already exist. We create the
// methods in a closure to avoid creating global variables.

if (!this.DocumentEcomProductGroup) {
	this.DocumentEcomProductGroup = {};
}

(function() {
	'use strict';

	var documentEcomProductGroupContextPath = location.protocol + '//'
			+ location.host + location.pathname;
	var documentEcomProductGroupModel = {
		documentPid : null
	};
	...
	function loadDocumentProductGroup(documentPid) {
		...
		documentEcomProductGroupModel.documentPid = documentPid;
		// clear all check box
		$.ajax({
			url : documentEcomProductGroupContextPath + "/" + documentPid,
			type : "GET",
	...
	function saveAssignedProductGroups() {
		...
		$.each($("input[name='productGroup']:checked"), function() {
			...
			documentProductGroups.push({
				documentPid : documentEcomProductGroupModel.documentPid,
		...
		$.ajax({
			url : documentEcomProductGroupContextPath + "/save",
			type : "POST",
	...
	function onSaveSuccess(result) {
		...
		window.location = documentEcomProductGroupContextPath;
	}

	DocumentEcomProductGroup.showModalPopup = function(el, id, action) {
		if (id) {
			switch (action) {
			case 0:
	...
	DocumentEcomProductGroup.closeModalPopup = function(el) {
		el.modal('hide');
	}
