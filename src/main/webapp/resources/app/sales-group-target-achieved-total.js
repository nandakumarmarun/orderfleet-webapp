...
(function() {
	...
	var salesGroupTargetAchievedTotalContextPath = location.protocol + '//'
	...
	function fillTargetData(targetData) {
       ...
       var achievedtotalRow ="";
		...
		$.each(targetData.finalList, function(salesTargetGroupName, targets) {
			...
			$.each(targets, function(index, targetSales) {
				
				$.each(targetSales, function(month, targets)
				 {
              
				
				var tVolume = targets.targetTotal;
				targetRow += "<td>"+ tVolume+"</td>";
				tTotal += tVolume;
				var tAmount = targets.achivedTotal;
				achivedRow += "<td>"+ tAmount+"</td>";
				console.log(tAmount);
				aTotal += tAmount;
				console.log(aTotal);
			...
			if(tTotal!=0 || aTotal!=0){
			performanceRow += "<tr><td>" + salesTargetGroupName + "</td>";
			performanceRow += "<td>"+tTotal.toFixed(2) +"</td>";
			performanceRow += "<td>"+aTotal.toFixed(2) +"</td>";
			
		
			var achivedPercentage = (aTotal / tTotal) * 100;
