...
(function() {
	...
	ProductGroup.setActive = function(name, productPid, isActivated) {
	...
	ProductGroup.setThirdpartyUpdate=function(name,pid,thirdpartyUpdate){
		productGroupModel.pid=pid;
	...
	ProductGroup.showModalPopup = function(el, id, action) {
		resetForm();
		if (id) {
			switch (action) {
			case 0:
...
//					$("#dbProductGroup1s").multiselect("refresh");
	...
	ProductGroup.loadProductGroups=function() {
		var selectedTaxMasters = "";
...
//					$("#dbProductGroup1s").multiselect("refresh");
	...
	ProductGroup.closeModalPopup = function(el) {
		el.modal('hide');
	}
