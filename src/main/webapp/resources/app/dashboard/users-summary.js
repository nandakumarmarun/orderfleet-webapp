...
function loadUsersSummaryByDashboardItemGroup(groupId, divUserPnlName) {
	...
	$.ajax({
		url : appContextPath + "/web/dashboard/users-summary/dashboarditem-group/" + groupId,
		type : "GET",
		data : {
...
function makeUserTileHeaderHtml(userData) {
	...
	let userProfileImg = userData.profileImage != null ? ('data:image/png;base64,' + userData.profileImage) : '/resources/assets/images/user.png';
	let attendanceSubGroup = userData.attendanceSubGroupCode != null ? userData.attendanceSubGroupCode : '';
	let empName = userData.userName != null ? userData.userName.substring(0, 13) : "";
	
	let html = "<div class='tile-menu'><div id='divUser"+ userData.userPid +"' class='tile-block " + getTileColor(userData.lastTime) + " height_tiles'>";
	html += "<div class='tile-header'><i id='attSubgroup" + userData.userPid + "'>"+ attendanceSubGroup +"</i>";
	html += getAttendanceThumbsHtml(userData);
	html += getUserNotification(userData.userPid, userData.notificationCount);
	html += "<img src='" + userProfileImg + "' width='30' height='30' class='img-circle profile_picture zoom'>";
	html += "<a href='"+ appContextPath + '/web/executive-task-executions?user-key-pid=' + userData.employeePid + "'>" +
					"<span class='text'>" + empName + "</span><span title='" + remarks + "' class='text' id='attRemark" + userData.userPid + "'>"+ remarks + "</span></a></div>";
	html += "<div id='divMonthSummary'>" + createSingleTile(userData.userSummaryData,userData.userPid, userData.employeePid) + "</div>";
	return html;
}
...
function makeUserTileFooterHtml(userData) {
	...
	let html = "<div class='tile-footer'><div id='clock" + userData.userPid + "' style='zoom: 0.39; -moz-transform: scale(0.5);display:none;margin-left: 8%;'></div>" +
			"<div id='location" + userData.userPid + "' style='display:block;'>" +
					"<p id='lastLoc" + userData.userPid + "' class='address popover-default' data-toggle='popover' data-trigger='hover' data-placement='top' data-content=\"$('#lastLoc" + userData.userPid + "').text()\" data-original-title='Address'>"; 
	html += lastLocation + "</p><a href='" + appContextPath + '/web/live-tracking?user-key-pid=' + userData.userPid +"' class='icon-container'><div class='icon' id='locIcon" + userData.userPid + "' >" + getLocationIcons(userData) + "</div></a>";
	html += "<p class='address'><font id='lastAccLoc" + userData.userPid + "'>" + lastAccountLocation + "</font></p>";
	html += "<p class='time'><font id='lastTime" + userData.userPid + "'>" + formatDate(userData.lastTime, 'HH:mm') + "</font></p>" + "</div></div></div></div>";
	return html;
}
...
function getTileColor(lastOrderTime) {
	...
	if (lastOrderTime != null) {
		...
		let orderDate = new Date(formatDate(lastOrderTime, 'MMM DD YYYY, h:mm:ss a'));
...
function getAttendanceThumbsHtml(userData) {
	...
	return "<a href='/web/attendance-report/all?empPid="+ userData.employeePid +"'><i id='attThumb" + userData.userPid + "' title='"+ attendanceTitle + "' class='entypo-thumbs-" + attendance + "'><span style='margin-left:20%;'>" + formatDate(userData.plannedDate, 'HH:mm') + "</span></i></a>";
}

function getUserNotification(userPid, notificationCount) {
	let dateAr = ($('#txtDate').val()).split('-');
	let newDate = dateAr[2] + '-' + dateAr[1] + '-' + dateAr[0];
	let skipped = "<a href='/web/day-plan-execution-report?pid="
			+ userPid + "&date=" + newDate + "' id='userNotification"
			+ userPid
			+ "'> <i class='entypo-attention'><span id='\skippedCount"
			+ userPid + "' class='error_status_number'>"
			+ notificationCount + "</span></i></a>";
