...
(function() {
	...
	CompanyUserDevice.releaseDevice = function(udPid) {
		if (confirm("Are you sure!")) {
			$.ajax({
				url : companyUserDeviceContextPath + "/" + udPid,
				type : 'PUT',
	...
	CompanyUserDevice.getCompanyUserDevice = function() {
		...
		if (companyPid != "no") {
			...
			$
					.ajax({
						url : companyUserDeviceContextPath
								+ "/getAllCompanyUserDeviceBySort",
						type : "GET",
						data : {
	...
	function activated(active) {
		var activatedLabel = "";
		if (active) {
			activatedLabel = '<label class="label label-success">True</label>'
		} else {
			activatedLabel = '<label class="label label-danger">False</label>'
		}
		return activatedLabel;
	}
	function spanReleasedDevice(companyUserDevicePid, activated) {

		var spanReleased = "";
		var companyUserDevice = "'" + companyUserDevicePid + "'";
		if (activated) {
			spanReleased = '<span class="label label-success" onclick="CompanyUserDevice.releaseDevice('
					+ companyUserDevice
					+ ')" style="cursor: pointer;">Release Device</span>';
		} else {
			spanReleased = "";
		}
		return spanReleased;
