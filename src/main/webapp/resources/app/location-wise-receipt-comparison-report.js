// Create a LocationWiseSalesComparisonReport object only if one does not already exist. We create the
// methods in a closure to avoid creating global variables.

if (!this.LocationWiseReceiptComparisonReport) {
	this.LocationWiseReceiptComparisonReport = {};
}

(function() {
	'use strict';

	var locationWiseReceiptComparisonReportContextPath = location.protocol + '//'
	...
	function loadLocationWiseTargetAchievedReport() {
		...
		$.ajax({
			url : locationWiseReceiptComparisonReportContextPath + "/load-data",
			type : 'GET',
			data : {
			...
			$.each(targetData.secondProductGroupLocationTargets, function(locationName2, targets2) {
				if (locationName === locationName2) {
					for(var i = 0, size = targets2.length; i < size ; i++) {
						var aAmount = targets2[i].achievedAmount;
						achivedTotal2 += aAmount;
						var aAmount1 = (aAmount).toFixed(2);
						achivedRow2 += "<td>"+ aAmount1 +"</td>";
					}
