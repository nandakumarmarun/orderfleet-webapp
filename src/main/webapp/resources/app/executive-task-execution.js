					...
					$
						.each(
							...
							function(index, executiveTaskExecution) {
								...
								if(executiveTaskExecution.locationType=="GpsLocation" && executiveTaskExecution.latitude==0 || executiveTaskExecution.locationType=="TowerLocation" && executiveTaskExecution.latitude==0 || executiveTaskExecution.location=="Unable to find location"){
									locationName="<span class='btn btn-success'  id='"+executiveTaskExecution.pid+"' onClick='ExecutiveTaskExecution.getLocation(this)' >get location</span>";
								}
//								if(executiveTaskExecution.locationType=="GpsLocation" && executiveTaskExecution.latitude==0 ||  executiveTaskExecution.location=="Unable to find location"){
//									locationName="<span class='btn btn-success'  id='"+executiveTaskExecution.pid+"' onClick='ExecutiveTaskExecution.getLocation(this)' >get location</span>";
//								}
								...
								if(distanceTarvel=="true"){
									geoTag="&nbsp;&nbsp;<button type='button' title='geo tag to account' class='btn btn-orange entypo-map' aria-label='entypo-map' onclick='ExecutiveTaskExecution.setGeoTag(\""+ executiveTaskExecution.pid+"\",\""+executiveTaskExecution.accountProfileLatitude+"\",\""+executiveTaskExecution.accountProfileLongitude+"\",\""+executiveTaskExecution.accountProfileLocation+"\",\""+executiveTaskExecution.latitude+"\",\""+executiveTaskExecution.longitude+"\",\""+executiveTaskExecution.location+"\",this);'></button>";
								}
								...
								$('#tBodyExecutiveTaskExecution')
									.append(
										...
										+ locationName
										+ locationIcon + "</i><br><label>Customer Location : </label><i id='lbl_"+ executiveTaskExecution.pid+ "'> "
										+ (executiveTaskExecution.accountProfileLocation == null ? "" : executiveTaskExecution.accountProfileLocation)
										+ "</i></td>"
										+ "<td>"
										+ (executiveTaskExecution.towerLocation == null ? "" :  executiveTaskExecution.towerLocation)
										+ "</td>"
										+ activityStatusTd
										...
										+ executiveTaskExecution.pid
										+ "\",this);'></button>&nbsp;&nbsp;<button type='button' title='reject' class='btn btn-red entypo-thumbs-down' aria-label='entypo-thumbs-down' onclick='ExecutiveTaskExecution.showModal(\"" + executiveTaskExecution.pid + "\",this);'></button>"+geoTag+changeAccount+"</td><td>"
										+ (executiveTaskExecution.remarks == null ? "" : executiveTaskExecution.remarks)
										+ "</td></tr>");
											...
											if (executionDetail.documentType != "DYNAMIC_DOCUMENT") {
												$(
													...
													.append(
														...
														+ "\",\""
														+ executionDetail.documentType
														+ "\");'>View Details</button>"+editButton+"</td></tr>");
											} else {
												var elmViewImage = "";
												...
												$(
													...
													.append(
														...
														+ "\",\""
														+ executionDetail.documentType
														+ "\");'>View Details</button>"+editButton+"</td>"
														+ elmViewImage
														+ "</tr>");
											}
										});
	...
	ExecutiveTaskExecution.setGeoTag= function(pid,accLat,accLon,accLoc,newLat,newLon,newLoc, obj){
		
		$("#oldLatitude").text((accLat == "null" || accLat == null ? "": accLat));
		$("#oldLongitude").text((accLon == "null" || accLon == null ? "": accLon));
		$("#oldGeoLocation").text((accLoc == "null" || accLoc == null? "": accLoc));
		
		$("#tblAccountProfileGeoLocation").html("<tr><input type='hidden' name='geoLocation' id='geoLocation' value='"+pid+"' >" 
				+"<td>"+newLat+"</td><td>"+newLon+"</td><td>"+newLoc+"</td></tr>");
	...
	ExecutiveTaskExecution.getActivities= function(activityType){
		...
		if(activityType=='all'){
			...
			$("#dbActivity").append('<option value="planed">All Planned Activity</option>');
		}else if(activityType=="unPlaned"){
			$("#dbActivity").append('<option value="unPlaned">All UnPlanned Activity</option>');
		}
		 $.ajax({
			 url : executiveTaskExecutionContextPath+"/getActivities/" + activityType,
			 method : 'GET',
	...
	ExecutiveTaskExecution.editDetails= function(pid,documentType,documentTypePid) {
		if(documentType=="INVENTORY_VOUCHER"){
		window.location=location.protocol + '//' + location.host
		+"/web/inventory-voucher-transaction?etePid="+pid+"&invPid="+documentTypePid;
		}else if(documentType=="DYNAMIC_DOCUMENT"){
			window.location=location.protocol + '//' + location.host
			+"/web/other-voucher-transaction?etePid="+pid+"&dynPid="+documentTypePid;
		}else if(documentType=="ACCOUNTING_VOUCHER"){
			window.location=location.protocol + '//' + location.host
			+"/web/accounting-voucher-transaction?etePid="+pid+"&accPid="+documentTypePid;
		}
	};
	...
	ExecutiveTaskExecution.changeAccountProfile= function(pid) {
		$
		.ajax({
			url : executiveTaskExecutionContextPath+ "/getDynamicDocument/" + pid,
			method : 'GET',
			async:false,
			success : function(data) {
				
				$("#field_dynamicDocument").html('<option value="-1">Select Dynamic Document</option>');
				if (data != null && data.length > 0) {
					$.each(data, function(index, dynamicDoc) {
						$("#field_dynamicDocument").append(
								'<option value=' + dynamicDoc.documentPid + '>'
								+ dynamicDoc.documentName + '</option>');
						$("#field_dynamicDocument").val(dynamicDoc.documentPid);
					});
				}
	...
	ExecutiveTaskExecution.createAndChangeAccount=function(){
		...
		accountProfile.defaultDiscountPercentage=$('#field_defaultDiscountPercentage').val();
		accountProfile.closingBalance=$('#field_closingBalance').val();
		
		$.ajax({
			url : executiveTaskExecutionContextPath + "/createAndChangeAccountProfile/"+savePidForAccountChanging+"/"+$("#field_territory").val(),
			type : "POST",
	...
	ExecutiveTaskExecution.getForms = function() {
		...
		$
				.ajax({
					url : executiveTaskExecutionContextPath+ "/getForms/" + dynamicDocumentPid,
					method : 'GET',
					success : function(data) {
	...
	function showDynamicDocumentImages(pid) {
		$('#divExecutiveTaskExecutionImages').html('<img src="/resources/assets/images/content-ajax-loader.gif" style="display: block; margin: auto;">');
		$.ajax({
			url : location.protocol + '//' + location.host + "/web/dynamic-documents/images/" + pid,
			type : 'GET',
		...
		$
			.ajax({
				...
				success : function(data) {
					...
					$('#lbl_userIc').text((data.userName == null ? "" : data.userName));
					$('#lbl_documentIc').text((data.documentName == null ? "" : data.documentName));
					$('#lbl_documentDateIc').text(
							formatDate(data.createdDate,'MMM DD YYYY, h:mm:ss a' ));
					$('#lbl_receiverIc').text((data.receiverAccountName == null ? "" : data.receiverAccountName));
					$('#lbl_supplierIc').text((data.supplierAccountName == null ? "" : data.supplierAccountName));
					$('#lbl_documentTotalIc').text((data.documentTotal == null ? "" : data.documentTotal));
					$('#lbl_documentDiscountAmountIc').text((data.docDiscountAmount == null ? "" : data.docDiscountAmount));
					$('#lbl_documentDiscountPercentageIc').text((data.docDiscountPercentage == null ? "" : data.docDiscountPercentage));
							...
							function(index, voucherDetail) {
								$('#tblVoucherDetailsIc')
									.append(
										...
										+ index
										+ "'><td>"
										+  (voucherDetail.productName == null ? "" : voucherDetail.productName)
										+ "</td><td>"
										+  (voucherDetail.quantity == null ? "" : voucherDetail.quantity)
										+ "</td><td>"
										+  (voucherDetail.freeQuantity == null ? "" : voucherDetail.freeQuantity)
										+ "</td><td>"
										+  (voucherDetail.sellingRate == null ? "" :voucherDetail.sellingRate)
										+ "</td><td>"
										+  (voucherDetail.taxPercentage == null ? "" :voucherDetail.taxPercentage)
										+ "</td><td>"
										+  (voucherDetail.discountAmount == null ? "" : voucherDetail.discountAmount)
										+ "</td><td>"
										+  (voucherDetail.discountPercentage == null ? "" : voucherDetail.discountPercentage)
										+ "</td><td>"
										+  (voucherDetail.remarks == null ? "" : voucherDetail.remarks)
										+ "</td><td>"
										+  (voucherDetail.rowTotal == null ? "" : voucherDetail.rowTotal)
										+ "</td></tr>");
								$
									.each(
										...
										function(index1,
											...
											$(
												...
												.append(
													...
													+  (inventoryVoucherBatch.batchNumber == null ? "" :inventoryVoucherBatch.batchNumber)
													+ "</td><td colspan='3'>Stock Location : "
													+  (inventoryVoucherBatch.stockLocationName == null ? "" : inventoryVoucherBatch.stockLocationName)
													+ "</td><td colspan='2'>Quantity : "
													+  (inventoryVoucherBatch.quantity == null ? "" : inventoryVoucherBatch.quantity)
													+ "</td></tr>");
										});
							});
	...
	function showAccountingVoucher(pid) {
		$
			.ajax({
				...
				method : 'GET',
				success : function(data) {
					$('#lbl_documentNumberAc').text((data.documentNumberLocal == null ? "" : data.documentNumberLocal));
					$('#lbl_userAc').text((data.userName == null ? "" : data.userName));
					$('#lbl_accountAc').text((data.accountProfileName == null ? "" : data.accountProfileName));
					$('#lbl_documentAc').text((data.documentName == null ? "" : data.documentName));
					$('#lbl_createdDateAc').text(
							formatDate(data.createdDate, 'MMM DD YYYY, h:mm:ss a'));
					$('#lbl_totalAmountAc').text((data.totalAmount == null ? "" : data.totalAmount));
					$('#lbl_outstandingAmountAc').text((data.outstandingAmount == null ? "" : data.outstandingAmount));
					$('#lbl_remarksAc').text((data.remarks == null ? "" : data.remarks));
							...
							function(index, voucherDetail) {
								...
								$('#tblVoucherDetailsAc')
									.append(
										...
										+ index
										+ "'><td>"
										+  (voucherDetail.amounte == null ? "" : voucherDetail.amount)
										+ "</td><td>"
										+  (voucherDetail.mode == null ? "" : voucherDetail.mode)
										+ "</td><td>"
										+  (voucherDetail.instrumentNumber == null ? "" : voucherDetail.instrumentNumber)
										+ "</td><td>"
										+ formatDate(voucherDetail.instrumentDate,'MMM DD YYYY' )
										+ "</td><td>"
										+  (voucherDetail.bankName == null ? "" : voucherDetail.bankName)
										+ "</td><td>"
										+  (voucherDetail.byAccountNamee == null ? "" : voucherDetail.byAccountName)
										+ "</td><td>"
										+  (voucherDetail.toAccountName == null ? "" : voucherDetail.toAccountName)
										+ "</td><td>"
										+ (voucherDetail.incomeExpenseHeadName == null ? "" : voucherDetail.incomeExpenseHeadName)
										+ "</td><td>"
										+  (voucherDetail.voucherNumber== null ? "" : voucherDetail.voucherNumber)
										+ "</td><td>"
										+ formatDate(voucherDetail.voucherDate,'MMM DD YYYY')
										+ "</td><td>"
									...
									.each(
										...
										function(index1,
											...
											if(voucherAllocation.remarks != null){
												remarks = (voucherAllocation.remarks).replace("~", "</td><td colspan='2'>");
											}
											$(
												'#tblVoucherDetailsAc')
												.append(
														...
														+ index
														+ "' >"
														+ "<td colspan='2'>Ref No : " +  (voucherAllocation.referenceNumber == null ? "" : voucherAllocation.referenceNumber)
														+ "<td colspan='2'>Payment Mode : " +  (voucherAllocation.mode == null ? "" : voucherAllocation.mode) + "</td>" 
														+ "<td colspan='2'>Amount : " + ( voucherAllocation.amount == null ? "" :  voucherAllocation.amount) + "</td>"
