...
(function() {
	...
	$(document).ready(function() {
		...
		$('#btnDownload').on('click', function() {
			...
			var name = "ready-to-dispatch-tsl"
			downloadXls1(name);
		});
	...
	function spanProcessFlowStatus(inventoryVoucherPid, status) {
		...
		case 'DEFAULT':
			spanProcessFlowStatus = '<div class="dropdown"><span class="label label-default dropdown-toggle" data-toggle="dropdown" style="cursor: pointer;">'
					...
					+ ','
					+ readyToDispatchAtPS
					+ ')" style="cursor: pointer;"><a>READY_TO_DISPATCH_AT_PS</a></li>'
					+ '</ul></div>';
			break;
		...
		case 'READY_TO_DISPATCH_AT_PS':
			spanProcessFlowStatus = '<div ><span class="label label-default dropdown-toggle" data-toggle="dropdown" >'
