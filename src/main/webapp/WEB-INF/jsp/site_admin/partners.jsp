...
<body class="page-body" data-url="">
	...
	<div class="page-container">
		...
		<div class="main-content">
					...
					<c:forEach items="${partners}" var="partner" varStatus="loopStatus">
						<tr class="${loopStatus.index % 2 == 0 ? 'odd' : 'even'}">
							<td><span style="cursor: pointer; color: #0072bc" title="click to view"
								onclick="Partner.showModalPopup($('#viewModal'),'${partner.pid}',0);">${partner.login== null ? "" : partner.login}</span></td>
							<td>${partner.firstName== null ? "" : partner.firstName}</td>
							<td>${partner.lastName== null ? "" : partner.lastName}</td>
							<td>${partner.address == null ? "" : partner.address}</td>
							<td>${partner.email == null ? "" : partner.email}</td>
							<td>${partner.mobile == null ? "" : partner.mobile}</td>
							...
							<td>${partner.pincode == null ? "" : partner.pincode}</td>
							<td>${partner.location == null ? "" : partner.location}</td>
							<td><span
								class="label ${partner.activated? 'label-success':'label-danger' }"
								onclick="Partner.setActive('${partner.login}','${partner.pid}','${ !partner.activated}')"
			...
			<spring:url value="/web/partners" var="urlPartner"></spring:url>
							...
							<div class="modal-body">
								...
								<div class="modal-body" style="overflow: auto; height: 60%;">
									<div class="form-group">
										<label class="control-label" for="field_name">First
									...
									<div class="form-group">
										<label class="control-label" for="field_name">Last
											Name</label> <input autofocus="autofocus" type="text"
			...
			<%-- <div class="modal fade container" id="enablePartnerModal">
				<!-- model Dialog -->
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">Enable Partner</h4>
						</div>
							...
							<div class="form-group">
								<div id="accountsCheckboxes">
									<table class='table table-striped table-bordered'>
										...
										<tbody id="tblEnablePartner">
											<c:forEach items="${deactivatedPartners}" var="partner">
												<tr>
													<td><input name='partner' type='checkbox'
														value="${partner.pid}" /></td>
													<td>${partner.name}</td>
												</tr>
											</c:forEach>
										</tbody>
						...
						<div class="modal-footer">
							<input class="btn btn-success" type="button"
								id="btnActivatePartner" value="Activate" />
	...
	<spring:url value="/resources/app/partner.js" var="partnerJs"></spring:url>
	<script type="text/javascript" src="${partnerJs}"></script>
</body>
</html>