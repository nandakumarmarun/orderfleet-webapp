...
<title>SalesNrich | Activities\Transactions</title>
...
<body class="page-body" data-url="">
		...
		<div class="main-content">
						...
						<div class="form-group">
								...
								<div class=" input-group">
									...
									<div class='dropdown-menu dropdown-menu-left'
										...
										<div>
											<a class='btn btn-default dropdown-item'
												style='width: 100%; text-align: left;'
												onclick='ExecutiveTaskExecution.getActivities("all")'>All
												Activity</a>
										</div>
										<div>
											<a class='btn btn-default dropdown-item'
												style='width: 100%; text-align: left;'
												onclick='ExecutiveTaskExecution.getActivities("planed")'>Planned
												Activity</a>
										</div>
										<div>
											<a class='btn btn-default dropdown-item'
												style='width: 100%; text-align: left;'
												onclick='ExecutiveTaskExecution.getActivities("unPlaned")'>UnPlanned
												Activity</a>
										</div>
							...
							<div class="col-sm-2">
								<br /> <select class="form-control" id="dbDateSearch"
									onchange="ExecutiveTaskExecution.showDatePicker()">
							...
							<div class="input-group col-sm-2">
								<div class="col-sm-3">
									...
									<button type="button" class="btn btn-info entypo-search"
										...
										onclick="ExecutiveTaskExecution.filter()" title="Apply"></button>
								</div>
								<div class="col-sm-3">
									<br />
			...
			<div class="table-responsive">
				<table class="collaptable table  table-striped table-bordered"
					...
					<!--table header-->
					<tbody id="tBodyExecutiveTaskExecution">
					</tbody>
				</table>
			</div>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div id="divExecutiveTaskExecutionImages"
			...
			<spring:url value="/web/executive-task-executions"
				var="urlExecutiveTaskExecutions"></spring:url>
		</div>
	</div>
	...
	<spring:url value="/resources/app/executive-task-execution.js"
		var="executiveTaskExecutionJs"></spring:url>
	<script type="text/javascript" src="${executiveTaskExecutionJs}"></script>
				...
				.ready(
						function() {
							...
							$('#myFormSubmit').on('click', function() {
								ExecutiveTaskExecution.reject();
							});
							$('#btnSaveNewGeoLocation').on('click', function() {
								ExecutiveTaskExecution.saveNewGeoLocation();
							});

							$('#btnSearch').click(
									function() {
										ExecutiveTaskExecution.searchTable($(
												"#search").val(),
												$('#tbodyAccountProfile'));
									});

							$('#newAccountProfile').click(function() {
								ExecutiveTaskExecution.newAccount();
							});

							$('#changeAccount').click(function() {
								ExecutiveTaskExecution.changeAccount();
							});
							$('#return').click(function() {
								ExecutiveTaskExecution.oldAccount();
							});
							...
							$('#loadAccountProfile').click(function() {
								ExecutiveTaskExecution.loadAccountFromForm();
							});
							...
							$('#btnDownload')
									.on(
											'click',
											function() {
												var tblExecutiveTaskExecution = $("#tblExecutiveTaskExecution tbody");
												if (tblExecutiveTaskExecution
														.children().length == 0) {
													alert("no values available");
													return;
												}
												if (tblExecutiveTaskExecution[0].textContent == "No data available") {
													alert("no values available");
													return;
												}
												ExecutiveTaskExecution
