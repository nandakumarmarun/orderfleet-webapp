...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
	...
	public MetricsConfiguration(OrderfleetProperties orderfleetProperties) {
	...
	public void init() {
		...
		if (orderfleetProperties.getMetrics().getJmx().isEnabled()) {
			log.debug("Initializing Metrics JMX reporting");
			JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
			jmxReporter.start();
		}
		if (orderfleetProperties.getMetrics().getLogs().isEnabled()) {
			log.info("Initializing Metrics Log reporting");
			final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
					.outputTo(LoggerFactory.getLogger("metrics")).convertRatesTo(TimeUnit.SECONDS)
					.convertDurationsTo(TimeUnit.MILLISECONDS).build();
			reporter.start(orderfleetProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
		}
	}
}