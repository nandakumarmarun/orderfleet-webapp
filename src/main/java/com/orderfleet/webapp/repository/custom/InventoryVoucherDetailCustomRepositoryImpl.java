...
public class InventoryVoucherDetailCustomRepositoryImpl implements InventoryVoucherDetailCustomRepository {
	...
	@Override
	public List<InventoryVoucherDetailDTO> getInventoryDetailListBy(List<String> productCategoryPids ,List<String> productGroupPids,
		...
		StringBuilder subQueryString = new StringBuilder("select "
		...
		if(!stockLocationPids.isEmpty()) {
			subQueryString.append(",ivd.sourceStockLocation.name,"
								+ "ivd.destinationStockLocation.name ");
		}
		...
		if(stockLocationPids.size() != 0 || !stockLocationPids.isEmpty()) {
			subQueryString.append(" and (ivd.sourceStockLocation.pid in :stockLocationPids or ivd.destinationStockLocation.pid in :stockLocationPids)");
		...
		if(productTerritoryPids.size() != 0 || !productTerritoryPids.isEmpty()) {
			subQueryString.append(" and ivd.inventoryVoucherHeader.receiverAccount in (select lap.accountProfile from LocationAccountProfile lap where lap.location.pid in :productTerritoryPids)");
		...
		for(Object[] object: objectArray){
			...
			if(!stockLocationPids.isEmpty()) {
				ivd.setSourceStockLocationName((String) object[8]);
				ivd.setDestinationStockLocationName((String) object[9]);
			}
