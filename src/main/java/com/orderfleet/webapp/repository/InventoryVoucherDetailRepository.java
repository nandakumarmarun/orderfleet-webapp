...
public interface InventoryVoucherDetailRepository extends JpaRepository<InventoryVoucherDetail, Long> {
	...
	Double sumOfAmountByUserIdAndDocumentsAndProductsAndCreatedDateBetween(Long userId,
			Set<Long> documentIds, Set<Long> productProfileIds, LocalDateTime fromDate,
			LocalDateTime toDate);
	...
	Double sumOfAmountByUserPidAndDocumentsAndProductsAndCreatedDateBetween(String userPid, Set<Long> documentIds, Set<Long> productProfileIds, LocalDateTime fromDate, LocalDateTime toDate);
	...
	@Query("select coalesce(sum(voucherDetail.quantity * coalesce(voucherDetail.product.unitQty, 1)),0) from InventoryVoucherDetail voucherDetail where voucherDetail.inventoryVoucherHeader.createdBy.id = ?1 and  voucherDetail.inventoryVoucherHeader.document.id in ?2 and voucherDetail.product.id in ?3 and voucherDetail.inventoryVoucherHeader.documentDate between ?4 and ?5")
	Double sumOfVolumeByUserIdAndDocumentsAndProductsAndCreatedDateBetween(Long userId,
			Set<Long> documentIds, Set<Long> productProfileIds, LocalDateTime fromDate,
			LocalDateTime toDate);
	...
	Double sumOfVolumeByUserPidAndDocumentsAndProductsAndCreatedDateBetween(String userPid, Set<Long> documentIds, Set<Long> productProfileIds, LocalDateTime fromDate, LocalDateTime toDate);
	...
	Double sumOfVolumeByUserPidAndDocumentsAndProductIdInAndDocumentDateBetween(String userPid, List<Document> documents,
	...
	Double sumOfAmountByDocumentsAndProductsAndAccountProfilesAndCreatedDateBetween(
			Set<Long> accountProfileIds, Set<Long> documents, Set<Long> productProfiles,
			LocalDateTime fromDate, LocalDateTime toDate);
	...
	List<CustomInventoryVoucherDetail> findAllByCompanyIdAndDateBetween(LocalDateTime fromDate,
			LocalDateTime toDate, List<Document> documents, List<String> employeePid);
	...
	List<Object[]> findAllByCompanyIdAndDocumentPidAndDateBetween(
			LocalDateTime fromDate, LocalDateTime toDate, String document, List<String> employeePid);
	...
	@Query(value = "select voucherDetail.inventoryVoucherHeader.employee.name, voucherDetail.product.pid, voucherDetail.product.name, voucherDetail.product.unitQty, voucherDetail.quantity,voucherDetail.inventoryVoucherHeader.receiverAccount.name, voucherDetail.inventoryVoucherHeader.documentDate, voucherDetail.inventoryVoucherHeader.executiveTaskExecution.remarks from InventoryVoucherDetail voucherDetail where voucherDetail.inventoryVoucherHeader.receiverAccount.pid in ?1 and voucherDetail.inventoryVoucherHeader.createdBy.id in ?2 and voucherDetail.inventoryVoucherHeader.document.id in ?3 and voucherDetail.inventoryVoucherHeader.documentDate between ?4 and ?5 and voucherDetail.product.activated=true")
