...
public interface InventoryVoucherDetailRepository extends JpaRepository<InventoryVoucherDetail, Long> {
	...
	@Query(value = "select voucherDetail.inventoryVoucherHeader.employee.name, voucherDetail.product.pid, voucherDetail.product.name, voucherDetail.product.unitQty, voucherDetail.quantity,voucherDetail.inventoryVoucherHeader.receiverAccount.name, voucherDetail.inventoryVoucherHeader.documentDate, voucherDetail.inventoryVoucherHeader.executiveTaskExecution.remarks,voucherDetail.inventoryVoucherHeader.receiverAccount.pid,voucherDetail.rowTotal,voucherDetail.inventoryVoucherHeader.document.name,voucherDetail.volume,voucherDetail.inventoryVoucherHeader.createdDate, voucherDetail.product.productDescription  from InventoryVoucherDetail voucherDetail where voucherDetail.inventoryVoucherHeader.receiverAccount.pid in ?1 and voucherDetail.inventoryVoucherHeader.createdBy.id in ?2 and voucherDetail.inventoryVoucherHeader.document.id in ?3 and voucherDetail.inventoryVoucherHeader.documentDate between ?4 and ?5 and voucherDetail.product.activated=true order by voucherDetail.inventoryVoucherHeader.documentDate desc")
	List<Object[]> findByAccountPidInAndEmployeeIdInAndDocumentIdInDateBetweenProduct(List<String> accountProfilePids,
			List<Long> userIds, List<Long> documentIds, LocalDateTime fromDate, LocalDateTime toDate);
	// Dash board Start
	...
	public String INVENTORY_DETAIL_LIST = "select \n" +
	...
	public String INVENTORY_DETAILS_LIST_SUMMERY = "select \n" +
			...
			"  and ivd.inventoryVoucherHeader.createdDate between ?2\n" +
			"  and ?3\n" +
			"  and ivd.inventoryVoucherHeader.document.pid in ?4\n" +
			"  and ivd.inventoryVoucherHeader.employee.pid in ?5\n" +
			"  and ivd.inventoryVoucherHeader.receiverAccount.pid in ?6\n";

	@Query(value = INVENTORY_DETAILS_LIST_SUMMERY)
	List<Object[]> getInventoryDetailListsummery(
			long companyId, LocalDateTime fromDate,
			LocalDateTime toDate,List<String> documentPids,
			List<String> employeePids, List<String> accountPids);


	public String INVENTORY_DETAILS_LIST_SUMMERY_ITEM_WISE = "select \n" +
			"  ivd.inventoryVoucherHeader.createdDate, \n" +
			"  ivd.inventoryVoucherHeader.employee.name, \n" +
			"  ivd.inventoryVoucherHeader.documentNumberServer, \n" +
			"  ivd.inventoryVoucherHeader.receiverAccount.name, \n" +
	...
	@Query(value = INVENTORY_DETAILS_LIST_SUMMERY_ITEM_WISE)
	List<Object[]> getInventoryDetailListsummeryitemwise(
			long companyId, LocalDateTime fromDate,
			LocalDateTime toDate,List<String> documentPids);
}
