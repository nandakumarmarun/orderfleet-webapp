...
public class OpeningStockExcelCochinDistributors {

	private final Logger log = LoggerFactory.getLogger(AccountProfileExcelCochinDistributors.class);
	...
	public ResponseEntity<String> bulkSaveOpeningStock(@Valid @RequestBody List<OpeningStockDTO> openingStockDTOs) {
		...
		if(!openingStockDTOs.isEmpty()) {
			...
			Optional<StockLocation> stockLocation = 
					stockLocations.stream().filter(sl -> sl.getName()!=null?sl.getName().equals(stockLocationName):false).findFirst();
