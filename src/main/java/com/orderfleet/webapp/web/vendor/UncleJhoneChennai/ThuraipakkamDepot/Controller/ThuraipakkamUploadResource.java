package com.orderfleet.webapp.web.vendor.UncleJhoneChennai.ThuraipakkamDepot.Controller;
...
import com.orderfleet.webapp.web.vendor.UncleJhoneChennai.ThuraipakkamDepot.Service.ThuraipakkamAccountsUploadService;
...
@Controller
@RequestMapping("/web")
public class ThuraipakkamUploadResource {

	private final Logger log = LoggerFactory.getLogger(ThuraipakkamUploadResource.class);

	@Inject
	private VyasarpadiProductsUploadService VyasarpadiProductsUploadService;

	@Inject
	private ThuraipakkamAccountsUploadService thuraipakkamAccountsUploadService;
	...
	static final String FACTORY_CODE = "TU";


	@RequestMapping(value = "/upload-unclejhon-chennai/thuraipakkam-accounts", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Void> uploadAccountProfiles() throws IOException, JSONException, ParseException {
		...
		if (accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ() != null
			...
			thuraipakkamAccountsUploadService
					.saveUpdateLocations(
			...
			thuraipakkamAccountsUploadService.saveAccountProfileGeoLocation(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ());
			...
			thuraipakkamAccountsUploadService.saveUpdateLocationAccounts(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ(),
					dealerResponse.getBody().getDealerUJ().getDealer());
		}
	...
	private ResponseEntity<DealerResponseUJ> TestMethodDealer() {
		...
		ResponseEntity<DealerResponseUJ> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3006/thuaraipakkam-dealer",
						HttpMethod.GET,
						Entity,
						DealerResponseUJ.class);
		return productProfileResponse;
	...
	private ResponseEntity<AccountProfileResponseUj> TestMethodCustomer() {
		...
		ResponseEntity<AccountProfileResponseUj> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3005/thuraipakam-customer",
						HttpMethod.GET,
						Entity,
						AccountProfileResponseUj.class);
		return productProfileResponse;
...
//	@RequestMapping(value = "/upload-unclejhon-chennai/thuraipakkam-products", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
//	@Timed
