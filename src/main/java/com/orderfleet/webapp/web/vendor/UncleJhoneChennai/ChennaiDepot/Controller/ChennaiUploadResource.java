package com.orderfleet.webapp.web.vendor.UncleJhoneChennai.ChennaiDepot.Controller;
...
import com.orderfleet.webapp.web.vendor.UncleJhoneChennai.ChennaiDepot.Services.ChennaiAccountsUploadService;
import com.orderfleet.webapp.web.vendor.UncleJhoneChennai.PallikkuppamDepot.Services.PallikkuppamAccountsUploadService;
...
@Controller
@RequestMapping("/web")
public class ChennaiUploadResource {

	private final Logger log = LoggerFactory.getLogger(ChennaiUploadResource.class);

	@Inject
	private VyasarpadiProductsUploadService VyasarpadiProductsUploadService;

	@Inject
	private ChennaiAccountsUploadService chennaiAccountsUploadService;
	...
	static final String FACTORY_CODE = "KC";


	@RequestMapping(value = "/upload-unclejhon-chennai/chennai-accounts", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Void> uploadAccountProfiles() throws IOException, JSONException, ParseException {
		...
		if (accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ() != null
			...
			chennaiAccountsUploadService.saveAccountProfileGeoLocation(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ());
			...
			chennaiAccountsUploadService.saveUpdateLocationAccounts(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ(),
					dealerResponse.getBody().getDealerUJ().getDealer());
		}
	...
	private ResponseEntity<DealerResponseUJ> TestMethodDealer() {
		...
		ResponseEntity<DealerResponseUJ> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3010/chennai-dealer",
						HttpMethod.GET,
						Entity,
						DealerResponseUJ.class);
		return productProfileResponse;
	...
	private ResponseEntity<AccountProfileResponseUj> TestMethodCustomer() {
		...
		ResponseEntity<AccountProfileResponseUj> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3009/chennai-customer",
						HttpMethod.GET,
						Entity,
						AccountProfileResponseUj.class);
		return productProfileResponse;
...
//	@RequestMapping(value = "/upload-unclejhon-chennai/chennai-products", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
//	@Timed
