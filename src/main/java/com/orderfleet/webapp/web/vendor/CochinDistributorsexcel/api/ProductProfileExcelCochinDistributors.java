...
public class ProductProfileExcelCochinDistributors {

	private final Logger log = LoggerFactory.getLogger(ProductProfileExcelCochinDistributors.class);
	...
	@RequestMapping(value = "/product-price.json", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<String> bulkSaveProductPrice(
			@Valid @RequestBody List<ProductProfileDTO> productProfileDTOs) {
		
		log.debug("REST request to save ProductPrice : {}", productProfileDTOs.size());
		return syncOperationRepository.findOneByCompanyIdAndOperationType(SecurityUtils.getCurrentUsersCompanyId(),
				SyncOperationType.PRODUCTPROFILE).map(so -> {
					// update sync status
					...
					syncOperationRepository.save(so);
					// save/update
					productProfileUploadService.saveUpdateProductPrice(productProfileDTOs, so);
					//productProfileUploadService.saveUpdateProductGroupProductExcel(productProfileDTOs, so);
					return new ResponseEntity<>("Uploaded", HttpStatus.OK);
				}).orElse(new ResponseEntity<>("Product-Profile sync operation not registered for this company",
						HttpStatus.BAD_REQUEST));
