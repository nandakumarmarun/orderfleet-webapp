package com.orderfleet.webapp.web.vendor.UncleJhoneChennai.TambaramDepot.Service;
...
@Service
public class ThambaramAccountsUploadService {
	String PID_PREFIX = "DDA-";

	final String FACTORY_CODE ="KT";
	final String FACTORY_NAME = "TAMBARAM DEPOT";



	private final Logger log = LoggerFactory.getLogger(ThambaramAccountsUploadService.class);
	...
	public ThambaramAccountsUploadService(BulkOperationRepositoryCustom bulkOperationRepositoryCustom,
			...
			for (AccountUJ apDto : accountUJ) {
				...
				if (!apDto.getLatitude().trim().isEmpty()) {
					...
					if (longitudeString.trim() != null
							...
							&& !longitudeString.trim().equalsIgnoreCase("NIL")
							&& !latitudeString.trim().equalsIgnoreCase( "NIL")) {
						try{
