...
@Service
public class AccountProfileUncleJhonHalasuruUploadService {

	String PID_PREFIX = "DDA-";

	final String FACTORY_CODE ="KI";

	final String FACTORY_NAME = "HALASURU DEPOT";

	private final Logger log = LoggerFactory.getLogger(AccountProfileUncleJhonHalasuruUploadService.class);
	...
	public AccountProfileUncleJhonHalasuruUploadService(BulkOperationRepositoryCustom bulkOperationRepositoryCustom,
...
//				}
	...
	public void saveUpdateLocationAccounts(List<AccountUJ> accountUJ, List<Dealer> dealer) {
		...
		List<LocationAccountProfile> locationAccountProfiles = locationAccountProfileRepository.findAllByCompanyId(companyId);
		...
		for (Dealer dealerLocAcc : dealer) {
			...
			Optional<AccountProfile> locAcc = accountProfiles.stream().filter(acc -> acc.getCustomerId().equalsIgnoreCase(dealerLocAcc.getDlrcode().trim())).findAny();

			Optional<Location> location = locations.stream().filter(pl -> pl.getName().equalsIgnoreCase(dealerLocAcc.getTown().trim())).findAny();
		...
		log.debug("Removing Existing Mapping in dealer" + locationAccountProfilesIds.size());
