package com.orderfleet.webapp.web.vendor.UncleJhoneChennai.PallikkuppamDepot.Controller;
...
@Controller
@RequestMapping("/web")
public class PallikkupamUploadResource {

	private final Logger log = LoggerFactory.getLogger(PallikkupamUploadResource.class);

	@Inject
	private VyasarpadiProductsUploadService VyasarpadiProductsUploadService;

	@Inject
	private PallikkuppamAccountsUploadService pallikkuppamAccountsUploadService;
	...
	static final String FACTORY_CODE = "KL";
	...
	private ResponseEntity<DealerResponseUJ> TestMethodDealer() {
		...
		ResponseEntity<DealerResponseUJ> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3008/pallikkuppam-dealers",
						HttpMethod.GET,
						Entity,
	...
	private ResponseEntity<AccountProfileResponseUj> TestMethodCustomer() {
		...
		ResponseEntity<AccountProfileResponseUj> productProfileResponse =
				restTemplate.exchange(
						"http://192.168.1.77:3007/pallikuppam-customer",
						HttpMethod.GET,
						Entity,
	...
	@RequestMapping(value = "/upload-unclejhon-chennai/pallikkupam-accounts", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Void> uploadAccountProfiles() throws IOException, JSONException, ParseException {
		...
		if (accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ() != null
			...
			pallikkuppamAccountsUploadService
					.saveUpdateLocations(
			...
			pallikkuppamAccountsUploadService.saveAccountProfileGeoLocation(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ());
			...
			pallikkuppamAccountsUploadService
					.saveDistributorDealerAssociation(
							dealerResponse.getBody().getDealerUJ().getDealer());

			pallikkuppamAccountsUploadService.saveUpdateLocationAccounts(
					accountProfileResponse.getBody().getAccountProfileUJ().getAccountUJ(),
					dealerResponse.getBody().getDealerUJ().getDealer());
		}
...
//@RequestMapping(value = "/upload-unclejhon-chennai/pallikkupam-products", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
//	@Timed
