...
public class PunchOutResource {
	...
	public ResponseEntity<List<PunchOutDTO>> filterPunchOutReport(@RequestParam("employeePid") String employeePid,
		...
		if (filterBy.equals("TODAY")) {
			punchOutList = getFilterData(employeePid, LocalDate.now(), LocalDate.now(), dateType);
		} else if (filterBy.equals("YESTERDAY")) {
			LocalDate yeasterday = LocalDate.now().minusDays(1);
			punchOutList = getFilterData(employeePid, yeasterday, yeasterday, dateType);
		} else if (filterBy.equals("WTD")) {
			TemporalField fieldISO = WeekFields.of(Locale.getDefault()).dayOfWeek();
			LocalDate weekStartDate = LocalDate.now().with(fieldISO, 1);
			punchOutList = getFilterData(employeePid, weekStartDate, LocalDate.now(), dateType);
		} else if (filterBy.equals("MTD")) {
			LocalDate monthStartDate = LocalDate.now().withDayOfMonth(1);
			punchOutList = getFilterData(employeePid, monthStartDate, LocalDate.now(), dateType);
		} else if (filterBy.equals("CUSTOM")) {
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
			LocalDate fromDateTime = LocalDate.parse(fromDate, formatter);
			LocalDate toDateTime = LocalDate.parse(toDate, formatter);
			punchOutList = getFilterData(employeePid, fromDateTime, toDateTime, dateType);
		}

		return new ResponseEntity<>(punchOutList, HttpStatus.OK);
	}

	private List<PunchOutDTO> getFilterData(String employeePid, LocalDate fDate, LocalDate tDate, String dateType) {
		...
		if (dateType.equalsIgnoreCase("punchOutDate")) {
			if (userPid.equals("no")) {
				List<Long> userIds = employeeHierarchyService.getCurrentUsersSubordinateIds();
				if (userIds.isEmpty()) {
					punchOutList = punchOutService.findAllByCompanyIdAndPunchDateBetween(fromDate, toDate);
				} else {
					punchOutList = punchOutService.findAllByCompanyIdUserPidInAndPunchDateBetween(userIds, fromDate,
							toDate);
				}
			} else if (!userPid.equals("no")) {
				punchOutList = punchOutService.findAllByCompanyIdUserPidAndPunchDateBetween(userPid, fromDate, toDate);
			}
		} else {
			if (userPid.equals("no")) {
				List<Long> userIds = employeeHierarchyService.getCurrentUsersSubordinateIds();
				if (userIds.isEmpty()) {
					punchOutList = punchOutService.findAllByCompanyIdAndCreatedDateBetween(fromDate, toDate);
				} else {
					punchOutList = punchOutService.findAllByCompanyIdUserPidInAndCreatedDateBetween(userIds, fromDate,
							toDate);
				}
			} else if (!userPid.equals("no")) {
				punchOutList = punchOutService.findAllByCompanyIdUserPidAndCreatedDateBetween(userPid, fromDate,
						toDate);
			}
		}
