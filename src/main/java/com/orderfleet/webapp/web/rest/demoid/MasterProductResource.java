package com.orderfleet.webapp.web.rest.demoid;
...
public class MasterProductResource {
	private final Logger log = LoggerFactory.getLogger(MasterProductResource.class);
	...
	@RequestMapping(value = "/product-categoriesid.json", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<String> bulkSaveProductCategories(
		...
		return syncOperationRepository.findOneByCompanyIdAndOperationType(SecurityUtils.getCurrentUsersCompanyId(),
				SyncOperationType.PRODUCTCATEGORY).map(so -> {
					...
					// save/update
					tpProductProfileManagementService.saveUpdateProductCategoriesId(productCategoryDTOs, so);
	...
	@RequestMapping(value = "/product-groupdataid.json", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<String> bulkSaveProductGroups(@Valid @RequestBody List<ProductGroupDTO> productGroupDTOs) {
		...
		return syncOperationRepository.findOneByCompanyIdAndOperationType(SecurityUtils.getCurrentUsersCompanyId(),
				SyncOperationType.PRODUCTGROUP).map(so -> {
					...
					syncOperationRepository.save(so);
					// save/update
					tpProductProfileManagementService.saveUpdateProductGroupsid(productGroupDTOs, so);
	...
	@RequestMapping(value = "/product-profileid.json", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<String> bulkSaveProductProfiles(
		...
		return syncOperationRepository.findOneByCompanyIdAndOperationType(SecurityUtils.getCurrentUsersCompanyId(),
				SyncOperationType.PRODUCTPROFILE).map(so -> {
					...
					syncOperationRepository.save(so);
					// save/update
					tpProductProfileManagementService.saveUpdateProductProfilesId(productProfileDTOs, so);
					return new ResponseEntity<>("Uploaded", HttpStatus.OK);
