...
public class CompanyConfigurationResource {
	...
	private List<CompanyConfigDTO> convertCompanyConfigurationToCompanyConfiguration(
		...
		groupedCompanyConfigurations.forEach((k, v) -> {
			...
			for (CompanyConfiguration cc : v) {
				...
				if (cc.getName().equals(CompanyConfig.SEND_EMAIL_AUTOMATICALLY)) {
				...
				if (cc.getName().equals(CompanyConfig.ENABLE_OUTSTANDING)) {
				...
				if (cc.getName().equals(CompanyConfig.ENABLE_DISATNCE_SLAB_CALC)) {
	...
	public ResponseEntity<String> saveCompanyConfiguration(@RequestParam String companyPid,
		...
		if (optmodernSpecialConfig.isPresent()) {
			...
			modernSpecialConfigCompany.setCompany(company);
			modernSpecialConfigCompany.setName(CompanyConfig.MODERN_SPECIAL_CONFIG);
			modernSpecialConfigCompany.setValue(modernSpecialConfig);
		}
		...
		if (optsendEmailAutomatically.isPresent()) {
			sendEmailAutomaticallyCompany = optsendEmailAutomatically.get();
			sendEmailAutomaticallyCompany.setValue(sendEmailAutomaticaly);
		} else {
			sendEmailAutomaticallyCompany = new CompanyConfiguration();
			sendEmailAutomaticallyCompany.setCompany(company);
			sendEmailAutomaticallyCompany.setName(CompanyConfig.SEND_EMAIL_AUTOMATICALLY);
			sendEmailAutomaticallyCompany.setValue(sendEmailAutomaticaly);
		}
		...
		if (optEnableoutstanding.isPresent()) {
			enableOutstandingCompany= optEnableoutstanding.get();
			enableOutstandingCompany.setValue(enableOutStanding);
		} else {
			enableOutstandingCompany = new CompanyConfiguration();
			enableOutstandingCompany.setCompany(company);
			enableOutstandingCompany.setName(CompanyConfig.ENABLE_OUTSTANDING);
			enableOutstandingCompany.setValue(enableOutStanding);
		}
		...
		if (optEnabledisatnceslabcalc.isPresent()) {
			enableDistanceSlabCalcCompnay= optEnabledisatnceslabcalc.get();
			enableDistanceSlabCalcCompnay.setValue(enableDistanceSlabCalc);
		} else {
			enableDistanceSlabCalcCompnay = new CompanyConfiguration();
			enableDistanceSlabCalcCompnay.setCompany(company);
			enableDistanceSlabCalcCompnay.setName(CompanyConfig.ENABLE_DISATNCE_SLAB_CALC);
			enableDistanceSlabCalcCompnay.setValue(enableDistanceSlabCalc);
		}
	...
	public ResponseEntity<Void> deleteCompanyConfiguration(@PathVariable String companyPid) throws URISyntaxException {
		...
		if (optsendTOFocus.isPresent()) {
			companyConfigurationRepository.deleteByCompanyIdAndName(optsendTOFocus.get().getCompany().getId(),
