...
public class PriceLevelAccountProductGroupServiceImpl implements PriceLevelAccountProductGroupService{
	...
	public PriceLevelAccountProductGroupDTO save(PriceLevelAccountProductGroupDTO priceLevelAccountProductGroupDTO) {
		...
		priceLevelAccountProductGroup.setPid(PriceLevelAccountProductGroupService.PID_PREFIX+RandomUtil.generatePid());
		...
		priceLevelAccountProductGroup.setProductGroup(productGroupRepository.findOneByPid(priceLevelAccountProductGroupDTO.getProductGroupPid()).get());
	...
	public PriceLevelAccountProductGroupDTO update(PriceLevelAccountProductGroupDTO priceLevelAccountProductGroupDTO) {
		log.debug("Request to save PriceLevelAccountProductGroup : {}", priceLevelAccountProductGroupDTO);
		PriceLevelAccountProductGroup priceLevelAccountProductGroup=priceLevelAccountProductGroupRepository.findOneByPid(priceLevelAccountProductGroupDTO.getPid()).get();
		priceLevelAccountProductGroup.setAccountProfile(accountProfileRepository.findOneByPid(priceLevelAccountProductGroupDTO.getAccountPid()).get());
		priceLevelAccountProductGroup.setPriceLevel(priceLevelRepository.findOneByPid(priceLevelAccountProductGroupDTO.getPriceLevelPid()).get());
		priceLevelAccountProductGroup.setProductGroup(productGroupRepository.findOneByPid(priceLevelAccountProductGroupDTO.getProductGroupPid()).get());
		priceLevelAccountProductGroup=priceLevelAccountProductGroupRepository.save(priceLevelAccountProductGroup);
	...
	public List<PriceLevelAccountProductGroupDTO> findAllByCompany() {
		...
		List<PriceLevelAccountProductGroup>priceLevelAccountProductGroups=priceLevelAccountProductGroupRepository.findAllByCompanyId();
	...
	public Optional<PriceLevelAccountProductGroupDTO> findOneByPid(String pid) {
		...
		return priceLevelAccountProductGroupRepository.findOneByPid(pid).map(priceLevelAccountProductGroup -> {
			PriceLevelAccountProductGroupDTO priceLevelAccountProductGroupDTO = new PriceLevelAccountProductGroupDTO(priceLevelAccountProductGroup);
	...
	public void saveMultiPriceLevelAccountProductGroup(String accountPids, String productGroupPids,
		...
		List<ProductGroup>productGroups=productGroupRepository.findAllByCompanyPidAndGroupPidIn(company.getPid(), Arrays.asList(productGroupPids.split(",")));
